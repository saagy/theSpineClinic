...
| `book_recurring_appointments(p_patient_id, p_type, p_slots, p_use_package, p_creator_id, p_doctor_ids, p_expected_next_visit_date)` | Books one or more slots with all doctors in a single transaction. When `p_expected_next_visit_date` is passed it re-verifies the due state inside the transaction so two receptionists cannot double-book the same recall (receptionist/super admin only). |
