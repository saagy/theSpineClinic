...
/// A skeleton matching clinical note cards: author header + 2 text lines.
...
/// Renders [count] skeleton note cards for records tabs.
class SkeletonNoteList extends StatelessWidget {
  const SkeletonNoteList({super.key, this.count = 4});
  final int count;
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemCount: count,
      itemBuilder: (context, index) => const SkeletonNoteCard(),
    );
  }
}
